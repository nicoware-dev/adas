...
import { extractMultipleProtocolNames, extractChainName, extractLimit } from "../../utils/extract";
...
const handler: Handler = async (runtime, message, state, _options, callback) => {
  try {
    ...
    // Check if this is a "top N protocols" query
    const isTopQuery = messageText.toLowerCase().includes("top") &&
                      (messageText.toLowerCase().includes("protocol") ||
                       messageText.toLowerCase().includes("protocols"));

    // Extract chain name if specified
    const chainName = extractChainName(messageText);
    elizaLogger.info(`Extracted chain: ${chainName || 'none'}`);

    // Extract limit if specified
    const limit = extractLimit(messageText) || 5;
    elizaLogger.info(`Extracted limit: ${limit}`);

    const protocols = extractMultipleProtocolNames(messageText);
    elizaLogger.info(`Extracted protocols: ${protocols.join(', ') || 'none'}`);

    // If no protocols found but this is a top query, we'll get the top N protocols
    if (protocols.length === 0 && !isTopQuery) {
      callback?.({
    ...
    // For "top N protocols" queries, get the top protocols by TVL
    if (isTopQuery || protocols.length === 0) {
      // Filter by chain if specified
      let filteredProtocols = allProtocols;
      if (chainName) {
        filteredProtocols = allProtocols.filter(p =>
          p.chains && p.chains.some(c => c.toLowerCase() === chainName.toLowerCase())
        );
        elizaLogger.info(`Filtered to ${filteredProtocols.length} protocols on ${chainName}`);
      }
      ...
      const foundProtocols: ProtocolTVLData[] = topProtocols.map(p => ({
        name: p.name,
        slug: p.slug,
        tvl: p.tvl || 0,
        chains: p.chains || [],
      ...
      // Generate response
      let response = chainName
        ? `# Top ${foundProtocols.length} Protocols on ${chainName}:\n\n`
        : `# Top ${foundProtocols.length} Protocols:\n\n`;

      foundProtocols.forEach((protocol, index) => {
        const chainPercentage = chainName && protocol.tvl > 0
          ? (protocol.chainTvls[chainName] / protocol.tvl * 100).toFixed(2)
          : null;

        response += `${index + 1}. ${protocol.name}: ${formatCurrency(protocol.tvl)}`;

        if (protocol.category) {
          response += ` (${protocol.category})`;
        }

        if (chainPercentage) {
          response += ` (${chainPercentage}% of total TVL)`;
        }

        if (protocol.change_1d) {
          const changeSign = protocol.change_1d >= 0 ? '+' : '';
          response += `\n   24h Change: ${changeSign}${protocol.change_1d.toFixed(2)}%`;
        }

        if (protocol.change_7d) {
          const changeSign = protocol.change_7d >= 0 ? '+' : '';
          response += `\n   7d Change: ${changeSign}${protocol.change_7d.toFixed(2)}%`;
        }
      ...
      callback?.({
        text: response,
        content: {
          action: "GET_MULTIPLE_PROTOCOL_TVL",
          status: "complete",
