...
function extractTokenSymbol(messageText: string): string {
  ...
  for (const token of commonTokens) {
    if (lowerText.includes(token)) {
      ...
      if (token === "aptos") return "apt";
      if (token === "wrapped aptos") return "wapt";
...
function extractMultipleTokenSymbols(messageText: string): string[] {
  ...
  if (commaMatch && commaMatch[1]) {
    ...
    for (const token of tokenList) {
      // Map common names to symbols
      if (token === "aptos") tokens.push("apt");
      else if (token === "wrapped aptos") tokens.push("wapt");
  ...
  return tokens.length > 0 ? tokens : ["btc", "eth", "apt"];
}
  ...
  [
    {
      user: "user1",
      content: {
        text: "What's the current price of APT?"
      }
    },
    {
      user: "assistant",
      content: {
        text: "The current price of APT (Aptos) is $0.75 USD. This represents a 2.5% increase in the last 24 hours. The current market cap is $2.5B with a 24h trading volume of $150M."
      }
    }
  ],
  [
