...
// Add formatCurrency as an alias for formatUSD
...
/**
 * Format pool information in a more concise way for trending pools
 */
export function formatPoolMarkdownConcise(poolInfo: PoolInfo): string {
    // Format the main pool information in a more concise way
    let markdown = `## ${poolInfo.name}\n`;
    markdown += `- TVL: ${formatUSD(poolInfo.tvl)}\n`;
    markdown += `- 24h Volume: ${formatUSD(poolInfo.volume24h)}\n`;

    // Add price change if available
    if (poolInfo.priceChange24h && poolInfo.priceChange24h !== 'N/A') {
        markdown += `- 24h Price Change: ${poolInfo.priceChange24h}\n`;
    }
    ...
    if (poolInfo.baseTokenName && poolInfo.quoteTokenName &&
        ...
        markdown += `- Prices: ${baseTokenName} (${formatUSD(poolInfo.baseTokenPrice)}), ${quoteTokenName} (${formatUSD(poolInfo.quoteTokenPrice)})\n`;
    }
