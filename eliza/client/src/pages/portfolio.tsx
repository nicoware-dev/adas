...
const COLORS = [
    '#01C0C9', // teal
    '#00FCB0', // light teal/green
    '#39D8E1', // light teal
    '#00787E', // dark teal
    '#A5F3FF', // very light teal
    '#10b981', // green
    '#ef4444', // red
    '#f59e0b', // amber
];
...
function getChainDisplayName(chainId: string): string {
    return CHAIN_NAMES[chainId.toLowerCase()] || 'Aptos';
}
...
export default function Portfolio() {
    ...
    if (!isConnected || !address) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[calc(100vh-3.5rem)] p-4">
                ...
                <h2 className="text-xl font-semibold mb-2">Connect Your Aptos Wallet</h2>
                <p className="text-muted-foreground text-center max-w-md mb-6">
                    Connect your wallet to view your Aptos portfolio balance and assets.
                </p>
            </div>
        );
    }
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[calc(100vh-3.5rem)] p-4">
                <Loader2 className="h-12 w-12 text-[#01C0C9] animate-spin mb-4" />
                <h2 className="text-xl font-semibold mb-2">Loading Portfolio</h2>
                <p className="text-muted-foreground text-center max-w-md">
                    Fetching your portfolio data from the Aptos blockchain...
                </p>
            </div>
        );
    }
    ...
    if (!portfolioData || !positionData) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[calc(100vh-3.5rem)] p-4">
                ...
                <p className="text-muted-foreground text-center max-w-md mb-6">
                    We couldn't find any assets in your Aptos wallet. If you believe this is an error, try refreshing the page.
                </p>
                <Button onClick={() => window.location.reload()}>
                    Refresh
                </Button>
    ...
    return (
        <div className="min-h-0 p-3 sm:p-4 md:p-6">
            {isMockData && (
                <div className="mb-4 p-3 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
                    <p className="text-sm text-yellow-400">
                        <strong>Note:</strong> Using mock data for development. In production, this will display real data from your Aptos wallet.
                    </p>
                </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4 md:gap-6 mb-4 sm:mb-6">
                ...
                <div className="p-4 sm:p-6 rounded-lg bg-white/[0.03] border border-white/[0.08]">
                    <div className="flex items-center gap-3 mb-4 sm:mb-6">
                        <div className="p-2 sm:p-3 rounded-full bg-[#01C0C9]/10">
                            <DollarSign className="h-5 w-5 sm:h-6 sm:w-6 text-[#01C0C9]" />
                        </div>
                        <div className="text-sm text-muted-foreground">Total Aptos Portfolio Value</div>
                    </div>
                ...
                <div className="p-4 sm:p-6 rounded-lg bg-white/[0.03] border border-white/[0.08]">
                    <div className="flex items-center gap-3 mb-4">
                        <div className="p-2 sm:p-3 rounded-full bg-[#01C0C9]/10">
                            <PieChart className="h-5 w-5 sm:h-6 sm:w-6 text-[#01C0C9]" />
                        </div>
                        <div className="text-sm text-muted-foreground">Token Distribution</div>
                    </div>
            ...
            <h3 className="text-lg font-medium mb-3">Your Aptos Assets</h3>
